...
x-api-key: sk_<your_store_api_key>`,
  },
  {
    title: "List Products",
    method: "GET",
    ...
    example: `curl "${BASE}/api/my-business/main-branch/products?page=1&limit=10"`,
  ...
  },
  {
    ...
    example: `curl -X POST "${BASE}/api/my-business/main-branch/check-price" \\
  ...
  },
  {
    ...
    example: `curl -X POST "${BASE}/api/my-business/main-branch/sales" \\
  -H "Content-Type: application/json" \\
  -H "x-api-key: sk_..." \\
  -d '{
    "items": [{"productId": "uuid", "quantity": 5}],
  ...
  },
  {
    ...
    example: `curl "${BASE}/api/my-business/main-branch/sales/ST-250601-ABC12" \\
  -H "x-api-key: sk_..."`,
    response: JSON.stringify({
      success: true,
      data: {
  ...
  },
  {
    ...
    example: `curl -X PATCH "${BASE}/api/my-business/main-branch/sales/ST-250601-ABC12" \\
  -H "Content-Type: application/json" \\
  -H "x-api-key: sk_..." \\
  -d '{
  ...
  },
  {
    ...
    example: `curl "${BASE}/api/my-business/main-branch/sales?paymentStatus=UNPAID" \\
  -H "x-api-key: sk_..."`,
    response: JSON.stringify({
      success: true,
      data: {
  ...
  },
  {
    ...
    example: `curl "${BASE}/api/my-business/main-branch/smart-baskets" \\
  -H "x-api-key: sk_..."`,
    response: JSON.stringify({
      success: true,
      data: {
  ...
  },
  {
    ...
    example: `curl "${BASE}/api/my-business/main-branch/smart-baskets/uuid" \\
  -H "x-api-key: sk_..."`,
    response: JSON.stringify({
      success: true,
      data: {
  ...
  },
  {
    ...
    optionalFields: ["customerName", "customerPhone", "discount", "paidAmount", "deliveryAddress", "notes"],
    example: `curl -X POST "${BASE}/api/my-business/main-branch/smart-baskets/uuid/buy" \\
  -H "Content-Type: application/json" \\
  -H "x-api-key: sk_..." \\
  -d '{
    "customerName": "ABC Traders",
  ...
  {
    title: "Error Format",
    desc: "All errors return a consistent format with a message.",
    example: `curl "${BASE}/api/invalid-business/main-branch/products"`,
    response: JSON.stringify({ success: false, error: "Business or branch not found" }, null, 2),
    isError: true,
  },
];

export default function StoreApiDocsPage() {
  return (
    <main className="relative pb-12">
      <div className="mb-8">
        <Link href="/store-api" className="mb-4 inline-flex items-center gap-1.5 text-xs font-medium text-(--clr-fg-muted) hover:text-(--clr-fg) transition-colors">
          <LuArrowLeft className="h-3.5 w-3.5" /> Back to Store API
        </Link>
        <div className="flex items-center gap-3 mt-2">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-(--clr-teal-dim)/10">
            <LuStore className="h-5 w-5 text-(--clr-teal-dim)" />
          </div>
          <div>
            <h1 className="text-2xl font-naston leading-tight text-(--clr-fg) sm:text-3xl">API Documentation</h1>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-8">
        {sections.map((section) => (
          <div key={section.title} className="rounded-xl border border-(--clr-border) bg-(--clr-surface) overflow-hidden">
            <div className="px-6 py-4 border-b border-(--clr-border)">
              <div className="flex items-center gap-3">
                ...
                <h2 className="font-semibold text-(--clr-fg)">{section.title}</h2>
              </div>
              {section.endpoint && (
                <p className="mt-1 font-mono text-xs text-(--clr-teal-dim)">{section.endpoint}{section.query || ""}</p>
              )}
            </div>
